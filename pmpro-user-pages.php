...
function pmproup_pmpro_after_checkout($user_id){
	...
	$user = get_userdata($user_id); 	// Get user info
	$level = pmpro_getMembershipLevelForUser($user_id); 	//get the user's level
	...
	if(in_array($level->ID, $options['levels'])) {
		...
		if(!$user_page_id) {
			...
			if($user_page_id)	{
				update_user_meta($user_id, "pmproup_user_page", $user_page_id);				//add meta
			}		
		}
...
function pmproup_pmpro_member_links_top(){
	...
	if(!empty($current_user->ID)) {
		...
		if($user_page_id) {
          ...
          //echo "<li><a href='get_permalink($page->ID)'>$page->post_title (date("m/d/Y", $page->post_date))</a></li>";
				?>
